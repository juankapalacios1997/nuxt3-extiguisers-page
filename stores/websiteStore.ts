...
import type { MyItem } from "~/interfaces/MyItem";
...
export const useWebsiteStore = defineStore('websiteStore', {
    ...
    getters: {
        getProductById: (state) => {
            return (id:number) => state.products.find(item => item.id === id) ?? null; 
        },
        getUserReviews: (state) => {
            return state.userReviews.map((i:any) => ({
                uuid: i.login.uuid, 
                name: `${i.name.first ?? "John"} ${i.name.last ?? "Doe"}`,
                username: i.login?.username ?? "Anonymous",
    ...
    actions: {
        async fetchUsersReviews() {
            ...
            const response:ResponseType = await $fetch("https://randomuser.me/api/?results=5", { method: 'GET' });

            if (!response){
                return;
            }

            this.userReviews = response.results;
        },
        addNewMyRental(id:number) {
            const myNewItem = this.products.find(item => item.id === id);

            if (!myNewItem) {
                return;
            }

            this.myRentals.push(myNewItem);
        }
    }
})